...
description: TLS, HIPAA, and security hardening
---
...
3. Use the **TLS Manager** web application (`https://<host>:8443/tls-manager/`) to manage certificates
...
### Password policy
Enforce strong passwords (see [User Management](./user_management.md)).

All password policy properties default to `0` (disabled). To enforce a strong policy, configure values in `mirth.properties`:
...
OIE supports extensible authentication through the `MultiFactorAuthenticationPlugin` abstract class, which extends `ServicePlugin`. After primary authentication succeeds, a registered MFA plugin can trigger secondary authentication via a client-side plugin.
